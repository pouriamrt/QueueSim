...
class Airport:
    ...
    def time_adv(self):
        t_next_event = min(self.t_next_arrival, self.t_departure1, self.t_departure2, self.t_departure3)  
        ...
        if self.t_next_arrival == min(self.t_next_arrival, self.t_departure1, self.t_departure2, self.t_departure3):
            self.arrival()
        elif self.t_departure1 == min(self.t_next_arrival, self.t_departure1, self.t_departure2, self.t_departure3):
            self.server1()
        elif self.t_departure2 == min(self.t_next_arrival, self.t_departure1, self.t_departure2, self.t_departure3):
            self.server2()
        else:
            self.server3()
...
while s.clock <= end_sim :
    ...
    print("Off times: ",round(s.total_off_s1,4)," - ",round(s.total_off_s2,4)," - ",round(s.total_off_s3,4))
    print("wait time in Queue: ",round(s.total_time_q1,4)," - ",round(s.total_time_q2,4)," - ",round(s.total_time_q3,4))
    print("[A,",round(s.t_next_arrival, 4), "],[C1,", round(s.t_departure1, 4), "],[C2,", round(s.t_departure2, 4), "],[C3,", round(s.t_departure3, 4),"]")
    print(s.num_in_q_s1," - ",s.num_in_q_s2," - ",s.num_in_q_s3)
    print(s.state_s1," - ",s.state_s2," - ",s.state_s3)
    print(s.total_passenger_s1, " - ", s.total_passenger_s2, " - ", s.total_passenger_s3)
    s.time_adv()
print("\n \nTotal number of Customers processed at Server 1: ", s.total_passenger_s1)
...
print("Avarage wait time in queue1: ",f'{s.total_time_q1/s.total_passenger_s1:9.4f}')
