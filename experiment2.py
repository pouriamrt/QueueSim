...
class Airport:
    def __init__(self, end_sim):
        ...
        self.total_passenger_s22 = 0
        self.total_passenger_s3 = 0
        self.total_off_s0 = 0
        self.total_off_s1 = 0
        self.total_off_s2 = 0
        self.total_off_s22 = 0
        self.total_off_s3 = 0
        self.off_s0 = 0
        self.off_s1 = 0
        self.off_s2 = 0
    ...
    def time_adv(self):
        t_next_event = min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3)  
        ...
        if self.t_next_arrival == min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3):
            self.arrival()
        elif self.t_departure0 == min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3):
            self.server0()
        elif self.t_departure1 == min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3):
            self.server1()
        elif self.t_departure2 == min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3):
            self.server2()
        elif self.t_departure22 == min(self.t_next_arrival, self.t_departure0, self.t_departure1, self.t_departure2, self.t_departure22, self.t_departure3):
            self.server22()
        else:
            self.server3()
    ...
    def server0(self):
        ...
        if self.num_in_q_s2 == 0:
            if self.state_s2 == 1 and self.state_s22 == 1:
                self.num_in_q_s2 += 1
            ...
            elif self.state_s22 == 0:
                self.state_s22 = 1
                self.total_off_s22 += (self.clock - self.off_s22)
                self.dep22 = self.gen_service_time_s2()
                self.t_departure22 = self.clock + self.dep22
                if self.t_departure22 >= self.break_times[0]:
                    self.t_departure22 += self.break_duration
                    print('break time at server2_2')
        else:
            self.num_in_q_s2 += 1
            self.t_q2 = self.clock
    ...
    def server22(self):
        if self.num_in_q_s2 > 0:
            ...
            self.total_time_q2 += (self.clock - self.t_q2)*self.num_in_q_s2
            self.num_in_q_s2 -= 1
            self.total_passenger_s22 += 1
            if self.t_departure22 >= self.break_times[0]:
                self.t_departure22 += self.break_duration
                print('break time at server2_2')
        else:
            self.total_passenger_s22 += 1
            self.off_s22 = self.clock
            self.t_departure22 = float('inf') 
            self.state_s22 = 0
...
while s.clock <= end_sim :
    ...
    s.time_adv()
print("\n \nTotal number of Customers processed at Server 1-1: ", s.total_passenger_s0)
print("Total number of Customers processed at Server 1-2: ", s.total_passenger_s1)
print("Total number of Customers processed at Server 2-1: ", s.total_passenger_s2)
print("Total number of Customers processed at Server 2-2: ", s.total_passenger_s22)
print("Total number of Customers processed at Server 3: ", s.total_passenger_s3,"\n")
print("Utilization of Server 1-1: ",f'{1-(s.total_off_s0/s.clock):9.4f}')
print("Utilization of Server 1-2: ",f'{1-(s.total_off_s1/s.clock):9.4f}')
print("Utilization of Server 2-1: ",f'{1-(s.total_off_s2/s.clock):9.4f}')
print("Utilization of Server 2-2: ",f'{1-(s.total_off_s22/s.clock):9.4f}')
...
print("Avarage wait time in queue2: ",f'{s.total_time_q2/(s.total_passenger_s2+s.total_passenger_s22):9.4f}')
print("Avarage wait time in queue3: ",f'{s.total_time_q3/s.total_passenger_s3:9.4f}',"\n")
print("Avarage service time at server 1-1: ",f'{((s.clock-s.total_off_s0)/s.total_passenger_s0):9.4f}')
print("Avarage service time at server 1-2: ",f'{((s.clock-s.total_off_s1)/s.total_passenger_s1):9.4f}')
print("Avarage service time at server 2-1: ",f'{((s.clock-s.total_off_s2)/s.total_passenger_s2):9.4f}')
print("Avarage service time at server 2-2: ",f'{((s.clock-s.total_off_s22)/s.total_passenger_s22):9.4f}')
